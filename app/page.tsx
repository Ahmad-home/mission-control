...
const EC: Record<Effort,string> = {
  light: 'text-emerald-400 bg-emerald-500/15 border-emerald-500/30',
  medium: 'text-amber-400 bg-amber-500/15 border-amber-500/30',
  heavy: 'text-red-400 bg-red-500/15 border-red-500/30',
}
...
export default function MC() {
  ...
  const [form,setForm]=useState({title:'',description:'',plan:'',notes:'',assignee:'Ahmad' as Assignee,priority:'medium' as Priority,status:'todo' as Status,tags:'',dueDate:'',effort:'medium' as Effort,progress:0})
  ...
  const openAdd=(s:Status='todo')=>{ setEditing(null); setForm({title:'',description:'',plan:'',notes:'',assignee:'Ahmad',priority:'medium',status:s,tags:'',dueDate:'',effort:'medium',progress:0}); setShowForm(true) }
    ...
    <div className="min-h-screen bg-[#0a0c11] text-white overflow-x-hidden" dir="rtl">
      <header className="sticky top-0 z-40 bg-[#111318]/90 backdrop-blur-xl border-b border-white/[0.06]">
        <div className="px-4 py-3">
          ...
          {view!=='calendar'&&(
            <div className="mt-3 bg-violet-500/10 border border-violet-500/20 rounded-xl p-2.5">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[10px] text-violet-300 font-medium">💜 سعة دانا</span>
                <span className={`text-[10px] font-bold ${capacity>=80?'text-red-400':capacity>=50?'text-amber-400':'text-emerald-400'}`}>{capacity}%</span>
              </div>
              <div className="h-2 bg-black/30 rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-500 ${capacity>=80?'bg-red-500':capacity>=50?'bg-amber-500':'bg-violet-500'}`} style={{width:`${Math.min(capacity,100)}%`}}/>
              </div>
              <div className="text-[9px] text-white/30 mt-1">{capacity>=80?'⚠️ حمل ثقيل — لا تضيف مهام جديدة':capacity>=50?'متوسط — ممكن مهمة خفيفة':'✨ متاح — ممكن مهام جديدة'}</div>
            </div>
          )}

          {view!=='calendar'&&(
            <div className="flex gap-1.5 mt-3 overflow-x-auto pb-0.5 -mx-4 px-4">
              {(Object.keys(PFL) as ProjectFilter[]).map(p=>(<button key={p} onClick={()=>setProj(p)} className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all border shrink-0 ${proj===p?'bg-violet-600 border-violet-500/50 text-white':'bg-white/[0.03] border-white/[0.06] text-white/40 hover:text-white/70 hover:border-white/20'}`}>{PFL[p]}</button>))}
            </div>
          )}

          {view!=='calendar'&&(
            <div className="flex flex-wrap gap-2 mt-3">
              <FG<'all'|Assignee> val={fA} onChange={v=>{setFA(v);sf2({fA:v})}} opts={[{v:'all',label:'الكل'},{v:'Ahmad',label:'👨 أحمد'},{v:'Dana',label:'💜 دانا'}]} />
              <FG<'all'|Priority> val={fP} onChange={v=>{setFP(v);sf2({fP:v})}} opts={[{v:'all',label:'الأولوية'},{v:'high',label:'🔴'},{v:'medium',label:'🟡'},{v:'low',label:'🟢'}]} />
              <FG<SortBy> val={sort} onChange={v=>{setSort(v);sf2({sort:v})}} opts={[{v:'dueDate',label:'📅 الموعد'},{v:'priority',label:'🔥 الأولوية'},{v:'createdAt',label:'🕐 الأقدم'}]} />
            </div>
          )}
        </div>
      </header>
      ...
      {view!=='calendar'&&(
        <div className="grid grid-cols-4 gap-2 px-4 py-3">
          {[{label:'جارٍ',v:ipT.length,ic:'⚡',c:'text-blue-400',bg:'bg-blue-500/10 border-blue-500/20'},{label:'عاجل',v:urgT.length,ic:'🔴',c:'text-red-400',bg:'bg-red-500/10 border-red-500/20'},{label:'أحمد',v:apf(tasks.filter(t=>t.assignee==='Ahmad'&&t.status!=='done')).length,ic:'👨',c:'text-amber-400',bg:'bg-amber-500/10 border-amber-500/20'},{label:'دانا',v:apf(tasks.filter(t=>t.assignee==='Dana'&&t.status!=='done')).length,ic:'💜',c:'text-violet-400',bg:'bg-violet-500/10 border-violet-500/20'}].map(s=>(
            <div key={s.label} className={`rounded-xl p-2.5 border flex items-center gap-2 ${s.bg}`}><span className="text-lg leading-none">{s.ic}</span><div className="min-w-0"><div className={`text-xl font-bold leading-none ${s.c}`}>{s.v}</div><div className="text-[9px] text-white/30 mt-0.5 truncate">{s.label}</div></div></div>
          ))}
        </div>
      )}
      {view==='focus'&&(
        <div className="px-4 pb-24">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <Col title="قيد التنفيذ" icon="⚡" top="border-t-blue-500" n={ipT.length} add={()=>openAdd('inprogress')}>
              ...
              {ipT.map(t=><TC key={t.id} task={t} onEdit={openEdit} onDel={del} onMove={move} onSetActive={setActive} onSetProgress={setProgress}/>)}
            </Col>
            <Col title="عاجل جداً" icon="🔴" top="border-t-red-500" n={urgT.length} add={()=>openAdd('todo')}>
              {urgT.length===0&&<E msg="لا يوجد شيء عاجل"/>}
              {urgT.map(t=><TC key={t.id} task={t} onEdit={openEdit} onDel={del} onMove={move} onSetActive={setActive} onSetProgress={setProgress}/>)}
            </Col>
          </div>
          {pendT.length>0&&(<div className="mt-4"><div className="flex items-center gap-2 mb-2 px-1"><span className="text-xs text-white/25">📥 قائمة الانتظار</span><div className="flex-1 h-px bg-white/[0.04]"/></div><div className="flex flex-col gap-1.5">{pendT.map(task=>{const due=fmtDue(task.dueDate);return(<div key={task.id} onClick={()=>openEdit(task)} className="bg-[#14171f] rounded-xl px-3 py-2.5 border border-white/[0.06] hover:border-white/15 cursor-pointer flex items-center gap-2.5 transition-all group"><span className="text-sm shrink-0">{PL[task.priority]}</span><span className="text-sm flex-1 min-w-0 truncate text-white/80 group-hover:text-white">{task.title}</span>{task.effort&&<span className={`text-[9px] px-1.5 py-0.5 rounded-full border shrink-0 ${EC[task.effort]}`}>{EL[task.effort]}</span>}{due&&<span className={`text-[10px] px-1.5 py-0.5 rounded-full border shrink-0 ${due.color}`}>{due.label}</span>}<span className="text-[10px] text-white/25 shrink-0">{task.assignee==='Ahmad'?'أحمد':'دانا'}</span><button onClick={e=>{e.stopPropagation();move(task.id,'inprogress')}} className="text-[10px] bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 px-2 py-1 rounded-lg transition-all shrink-0 opacity-0 group-hover:opacity-100">⚡ ابدأ</button></div>)})}</div></div>)}
          <div className="mt-4"><button onClick={()=>setArchive(!archive)} className="w-full py-2.5 rounded-xl border border-dashed border-white/[0.06] text-xs text-white/25 hover:text-white/40 hover:border-white/15 transition-all">{archive?'✖ إخفاء المكتملة':`✔ عرض المكتملة (${done.length})`}</button>{archive&&(<div className="mt-3 flex flex-col gap-1.5">{done.map(t=>(<div key={t.id} onClick={()=>openEdit(t)} className="flex items-center gap-2.5 px-3 py-2.5 bg-white/[0.02] rounded-xl border border-white/[0.04] cursor-pointer opacity-50 hover:opacity-70"><span className="text-sm">✅</span><span className="text-sm flex-1 line-through text-white/40 truncate">{t.title}</span><span className="text-[10px] text-white/20">{t.assignee==='Ahmad'?'أحمد':'دانا'}</span></div>))}</div>)}</div>
        </div>
      )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <DroppableCol id="col-todo" title="للبدء" icon="📌" top="border-t-slate-500" n={todoT.length} add={()=>openAdd('todo')}>
                <SortableContext items={todoT.map(t=>t.id)} strategy={verticalListSortingStrategy}>
                  ...
                  {todoT.map(t=><DraggableTC key={t.id} task={t} onEdit={openEdit} onDel={del} onMove={move} onSetActive={setActive} onSetProgress={setProgress}/>)}
                </SortableContext>
              </DroppableCol>
              <DroppableCol id="col-inprogress" title="قيد التنفيذ" icon="⚡" top="border-t-blue-500" n={ipT.length} add={()=>openAdd('inprogress')}>
                <SortableContext items={ipT.map(t=>t.id)} strategy={verticalListSortingStrategy}>
                  ...
                  {ipT.map(t=><DraggableTC key={t.id} task={t} onEdit={openEdit} onDel={del} onMove={move} onSetActive={setActive} onSetProgress={setProgress}/>)}
                </SortableContext>
              </DroppableCol>
              <DroppableCol id="col-done" title="مكتمل" icon="✅" top="border-t-emerald-500" n={done.length} add={()=>{}}>
                <SortableContext items={done.map(t=>t.id)} strategy={verticalListSortingStrategy}>
                  ...
                  {done.map(t=><DraggableTC key={t.id} task={t} onEdit={openEdit} onDel={del} onMove={move} onSetActive={setActive} onSetProgress={setProgress} isDone/>)}
                </SortableContext>
              </DroppableCol>
            </div>
          </div>
          <DragOverlay>{draggedTask && <TC task={draggedTask} onEdit={()=>{}} onDel={()=>{}} onMove={()=>{}} onSetActive={()=>{}} onSetProgress={()=>{}} isDragging/>}</DragOverlay>
        </DndContext>
      )}
      ...
      {showForm&&(
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end md:items-center justify-center z-50" onClick={e=>e.target===e.currentTarget&&setShowForm(false)}>
          <div className="bg-[#111318] border border-white/[0.08] rounded-t-3xl md:rounded-2xl w-full md:max-w-lg max-h-[92vh] overflow-y-auto shadow-2xl">
            <div className="sticky top-0 bg-[#111318] px-5 pt-5 pb-4 border-b border-white/[0.06] flex items-center justify-between">
              ...
              <button onClick={()=>setShowForm(false)} className="w-8 h-8 rounded-full bg-white/[0.05] hover:bg-white/10 flex items-center justify-center text-white/40 hover:text-white transition-all text-sm">✕</button>
            </div>
            <div className="p-5 flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <F lbl="👤 المسؤول"><select value={form.assignee} onChange={e=>setForm({...form,assignee:e.target.value as Assignee})} className="w-full bg-[#1a1d26] border border-white/[0.08] rounded-xl px-4 py-3 text-sm outline-none focus:border-violet-500/50 text-white transition-all [&>option]:bg-[#1a1d26] [&>option]:text-white"><option value="Ahmad">👨 أحمد</option><option value="Dana">💜 دانا</option></select></F>
                <F lbl="🎯 الأولوية"><select value={form.priority} onChange={e=>setForm({...form,priority:e.target.value as Priority})} className="w-full bg-[#1a1d26] border border-white/[0.08] rounded-xl px-4 py-3 text-sm outline-none focus:border-violet-500/50 text-white transition-all [&>option]:bg-[#1a1d26] [&>option]:text-white"><option value="high">🔴 عاجل</option><option value="medium">🟡 متوسط</option><option value="low">🟢 منخفض</option></select></F>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <F lbl="⏱ الجهد المطلوب"><select value={form.effort} onChange={e=>setForm({...form,effort:e.target.value as Effort})} className="w-full bg-[#1a1d26] border border-white/[0.08] rounded-xl px-4 py-3 text-sm outline-none focus:border-violet-500/50 text-white transition-all [&>option]:bg-[#1a1d26] [&>option]:text-white"><option value="light">⚡ خفيف (&lt;30m)</option><option value="medium">⏱ متوسط (30m-2h)</option><option value="heavy">🔥 ثقيل (2h+)</option></select></F>
                <F lbl="📊 الحالة"><select value={form.status} onChange={e=>setForm({...form,status:e.target.value as Status})} className="w-full bg-[#1a1d26] border border-white/[0.08] rounded-xl px-4 py-3 text-sm outline-none focus:border-violet-500/50 text-white transition-all [&>option]:bg-[#1a1d26] [&>option]:text-white"><option value="todo">📌 للبدء</option><option value="inprogress">⚡ قيد التنفيذ</option><option value="done">✅ مكتمل</option></select></F>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <F lbl="📅 تاريخ الاستحقاق"><input type="date" value={form.dueDate} onChange={e=>setForm({...form,dueDate:e.target.value})} className="w-full bg-[#1a1d26] border border-white/[0.08] rounded-xl px-4 py-3 text-sm outline-none focus:border-violet-500/50 text-white transition-all [color-scheme:dark]"/></F>
              ...
              <F lbl="🏷 تاغات مفصولة بفاصلة"><input value={form.tags} onChange={e=>setForm({...form,tags:e.target.value})} placeholder="Sonika, Audiom, ..." className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3 text-sm outline-none focus:border-violet-500/50 transition-all placeholder:text-white/20"/></F>
            </div>
            <div className="flex gap-2.5 px-5 pb-5">
              <button onClick={submit} className="flex-1 bg-violet-600 hover:bg-violet-500 text-white font-semibold py-3.5 rounded-xl transition-all text-sm">{editing?'حفظ التعديلات':'إضافة المهمة'}</button>
              {editing&&(<button onClick={()=>{del(editing.id);setShowForm(false)}} className="px-4 bg-red-500/10 hover:bg-red-500/20 text-red-400 font-medium py-3.5 rounded-xl transition-all text-sm border border-red-500/20">حذف</button>)}
              <button onClick={()=>setShowForm(false)} className="px-5 bg-white/[0.04] hover:bg-white/[0.08] text-white/50 font-medium py-3.5 rounded-xl transition-all text-sm">إلغاء</button>
            </div>
          </div>
        </div>
      )}
...
function TC({task,onEdit,onDel,onMove,onSetActive,onSetProgress,isDone,isDragging}:{task:Task;onEdit:(t:Task)=>void;onDel:(id:string)=>void;onMove:(id:string,s:Status)=>void;onSetActive:(id:string,active:boolean)=>void;onSetProgress:(id:string,p:number)=>void;isDone?:boolean;isDragging?:boolean}) {
  ...
  return (
    <div className={`rounded-xl p-3 border transition-all cursor-pointer ${isDragging?'opacity-70 scale-105 shadow-2xl':''} ${isDone?'opacity-50 bg-white/[0.015] border-white/[0.04]':ov?'bg-[#14171f] border-red-500/25 hover:border-red-500/40':'bg-[#14171f] border-white/[0.06] hover:border-white/15'} ${activeClass}`} onClick={()=>onEdit(task)}>
      ...
      <div className="flex items-start gap-2 mb-2"><h3 className={`flex-1 font-medium text-sm leading-snug min-w-0 ${isDone?'line-through text-white/30':'text-white/90'}`}>{task.title}</h3><button onClick={e=>{e.stopPropagation();onDel(task.id)}} className="w-5 h-5 rounded-lg text-white/15 hover:text-red-400 hover:bg-red-500/10 flex items-center justify-center transition-all shrink-0 text-base leading-none">×</button></div>
...
function DraggableTC({task,onEdit,onDel,onMove,onSetActive,onSetProgress,isDone}:{task:Task;onEdit:(t:Task)=>void;onDel:(id:string)=>void;onMove:(id:string,s:Status)=>void;onSetActive:(id:string,active:boolean)=>void;onSetProgress:(id:string,p:number)=>void;isDone?:boolean}) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: task.id })
  const style = { transform: CSS.Transform.toString(transform), transition }
  return (<div ref={setNodeRef} style={style} {...attributes} {...listeners}><TC task={task} onEdit={onEdit} onDel={onDel} onMove={onMove} onSetActive={onSetActive} onSetProgress={onSetProgress} isDone={isDone} isDragging={isDragging}/></div>)
}
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <button onClick={()=>{if(m===0){setM(11);setY(y-1)}else setM(m-1)}} className="px-3 py-2 bg-white/[0.04] hover:bg-white/10 rounded-xl text-sm transition-all">←</button>
        <h2 className="font-bold">{mn[m]} {y}</h2>
        <button onClick={()=>{if(m===11){setM(0);setY(y+1)}else setM(m+1)}} className="px-3 py-2 bg-white/[0.04] hover:bg-white/10 rounded-xl text-sm transition-all">→</button>
      </div>
